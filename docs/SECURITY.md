...
- `reconsolidation-strong-preflight` verifies a backup, restores it into a temporary shadow store, and runs prepare/apply/review plus optional recall regression only in that restored copy. It redacts the shadow approval token from output and does not approve live promotion, supersession, rewrite, deletion, or cooling.
