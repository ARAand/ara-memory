...
- `provenance-compact` is dry-run by default and applies only with `--apply --confirm "COMPACT PROVENANCE"`. Apply mode rewrites active source-event links in one transaction with compare-and-set checks on capsule status, expected source IDs, and retained event existence; any stale or malformed item aborts the whole batch. With recall-regression manifest/baseline inputs, baseline-visible and baseline-selected capsules are protected, then apply simulates the remaining rewrite in a shadow store, elides unstable candidates that shift reviewed recall paths, and blocks live mutation when no recall-stable plan remains. Successful writes leave `memory_actions` rows with `action=compact-provenance` and `actor=provenance-compaction`.
