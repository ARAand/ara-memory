...
- `relation-merge-review` is read-only. It audits applied merge witnesses for candidate remnants, remaining candidate edge references, self-loop residue, duplicate coalescing, and evidence-count preservation before later automation is allowed to trust relation-node merges. When supplied with recall-regression manifest/baseline paths, it also runs those reviewed cases as a sandbox and fails the combined gate without mutating capsules, source events, relation graph rows, or review state.
