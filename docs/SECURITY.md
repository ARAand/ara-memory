...
- `reconsolidation-frame` and `reconsolidation-prepare` are read-only. They can gather purpose, identity, settled-decision, failure/conflict, working-context, and forgetting-boundary evidence, but cannot mutate memory. Prepare stores a rollback witness preview for evidence capsule identities, source links, and content digests.
- `reconsolidation-apply` is candidate-only. It consumes a short-lived approval token once, rechecks the frame fingerprint and prepared rollback witness fingerprint, creates at most one candidate summary capsule, and writes a witness. It cannot promote, supersede, rewrite, delete, or cool existing capsules.
- `reconsolidation-backfill-rollback-witnesses` is an explicit legacy repair path, not an auto-pass. It dry-runs by default, reconstructs only missing approval rollback witnesses from the immutable reconsolidation witness `before_json` snapshot, records an audit action when applied, and still requires `reconsolidation-review --record-queue` to resolve the queue.
- `reconsolidation-strong-preflight` verifies a backup, restores it into a temporary shadow store, and runs prepare/apply/review plus optional recall regression only in that restored copy. It redacts the shadow approval token from output and does not approve live promotion, supersession, rewrite, deletion, or cooling. `prepare-live-reconsolidation-action` may store a short-lived one-use approval record for exactly one design-ready action, but it still does not mutate live memory and still requires `live_authorized=false`; any future executor must consume the token once and recheck backup identity, compare-and-set target state, and rollback or exception witness evidence. `live-reconsolidation-cool` is the first narrow executor: it consumes only a prepared cool approval, blocks core lifecycle anchors, compare-and-sets exactly one active capsule to `superseded`, preserves source events and capsule text, and writes a `reconsolidation_action_witnesses` row. `reconsolidation-action-witnesses` reviews those rows against the allowed field-transition policy and fails on later target drift before stronger live executors are opened. `reconsolidation-action-shadow-rollback` restores a verified backup and rolls back only reviewed cool witnesses in that temporary copy; it is evidence for a future approval gate, not live rollback authority. `prepare-live-reconsolidation-action-rollback` may store a short-lived one-use approval record only after that shadow rollback passes for exactly one live witness and the live witness still passes review. `reconsolidation-action-rollback-approvals` is read-only and rechecks prepared approval expiry, backup identity, backup verification, live witness review, and target snapshot drift without consuming tokens. `live-reconsolidation-action-rollback` consumes one prepared rollback approval, reruns that review, compare-and-sets only a recorded `cool` status transition from `superseded` back to the prior active status, and writes `reconsolidation_action_rollback_witnesses`; it does not restore text, source events, arbitrary fields, or non-`cool` actions.
- `reconsolidation-shadow-rollback` verifies a backup and restores it into a temporary shadow store before applying rollback. It rejects only the candidate frame capsule created by a reconsolidation witness, preserves evidence/source/witness rows, and runs doctor afterward without mutating the live store. `prepare-live-reconsolidation-rollback` and `live-reconsolidation-rollback` open the live path only for one explicit witness with a short-lived one-use token, exact confirmation, backup identity recheck, witness invariant recheck, candidate capsule digest recheck, and a new rollback witness row. It does not promote, rewrite, delete, or cool arbitrary memory.
- `reconsolidation-exception-witness` does not grant broad mutation authority. It only explains one reviewed capsule field transition by witness id, capsule id, field, and exact before/after digest; unmatched drift still fails reconsolidation review and live rollback preparation/execution.
...
- `relation-merge-apply` consumes short-lived prepared approvals once. It reruns the approved dry-run parameters, compares the relation fingerprint and candidate snapshot, requires exact confirmation, rewires only approved relation-node aliases, coalesces duplicate relation edges, marks the approval used, and writes rollback witnesses in the same SQLite transaction. It does not mutate capsules or source events.
- `relation-merge-review` is read-only unless `--record-queue` is explicitly supplied. It audits applied merge witnesses for candidate remnants, remaining candidate edge references, self-loop residue, duplicate coalescing, and evidence-count preservation before later automation is allowed to trust relation-node merges. When supplied with recall-regression manifest/baseline paths, it also runs those reviewed cases as a sandbox and fails the combined gate without mutating capsules, source events, or relation graph rows. `--record-queue` writes only relation-specific review queue rows for watch/fail signals or regression failures, and passing re-reviews resolve stale queue rows. The health `relation_review_pressure` signal is read-only: it lists open relation-review queue rows, fails on open `fail` rows, warns on open `watch` rows, and records nothing unless the review command is later run with explicit `--record-queue`.
- `global-spreading-sandbox` is read-only. It runs recall-plan probes with global memory enabled and inspects aggregate diagnostics for fanout, supplementation, depth, visible evidence, low-evidence fallback, and risk-filter pressure. It does not render raw graph paths, mutate capsules, write relation rows, or promote global memories. Missing global evidence is watch-level capability pressure; excess fanout, excess supplementation, or paths beyond the configured depth are failures because they can turn cross-scope recall into accidental always-on context.
...
- `cold-map` is read-only distant-memory navigation. It may scan cold capsule text locally for query matches, but it renders only redacted examples, group counts, lifecycle tiers, and source-event digests; cold bodies do not re-enter active recall or advisor payloads.
- `recall-policy` is a local, read-only recall controller. It classifies the query before context is rendered, chooses hot/core, working-memory, recall-context, cold-map, or retention-cycle actions, and keeps distant cold bodies out of model context.
- `recall-policy-impact` records policy outcomes as append-only local evidence and requires the intent, strategy, and action names from the policy decision actually used. `recall-policy-eval` only aggregates that evidence, and roadmap feedback gates use scope-local evidence with a minimum reviewed-outcome threshold. The aggregate does not automatically change routing, promotion, pruning, or hot-memory eligibility, which keeps feedback from becoming a reward-hacking channel.
- `agency-review` is a bounded judgment gate, not an autonomous executor. It can record a stance and reasons as an append-only audit note, but it does not run commands, mutate routing, promote memories, prune memories, or override user/system/developer instructions. Anti-judgment frames are treated as reasons to refuse or reframe rather than as higher-priority instructions. `passed` means the review completed with sufficient evidence; automation should use `action_allowed` or `--strict-action-exit` when it needs permission to proceed. Agency-review records must not become evidence that agency itself is trustworthy; they are audit-only notes, not purpose, identity, promotion, or self-certification evidence.
- `provenance-compact` is dry-run by default and applies only with `--apply --confirm "COMPACT PROVENANCE"`. Apply mode rewrites active source-event links and records provenance witnesses in one transaction with compare-and-set checks on capsule status, expected source IDs, and retained event existence; any stale or malformed item aborts the whole batch. Witnesses preserve the original source-event set, retained set, counts, and digests for audit after direct active links are shortened. With recall-regression manifest/baseline inputs, baseline-visible and baseline-selected capsules are protected, then apply simulates the remaining rewrite in a shadow store, elides unstable candidates that shift reviewed recall paths, and blocks live mutation when no recall-stable plan remains. Successful writes leave `memory_actions` rows with `action=compact-provenance` and `actor=provenance-compaction`.
