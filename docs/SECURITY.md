...
- `reconsolidation-frame` and `reconsolidation-prepare` are read-only. They can gather purpose, identity, settled-decision, failure/conflict, working-context, and forgetting-boundary evidence, but cannot mutate memory.
- `reconsolidation-apply` is candidate-only. It consumes a short-lived approval token once, rechecks the frame fingerprint, creates at most one candidate summary capsule, and writes a witness. It cannot promote, supersede, rewrite, delete, or cool existing capsules.
