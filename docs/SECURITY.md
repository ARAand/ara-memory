...
- `reconsolidation-shadow-rollback` verifies a backup and restores it into a temporary shadow store before applying rollback. It rejects only the candidate frame capsule created by a reconsolidation witness, preserves evidence/source/witness rows, runs doctor afterward, and never mutates the live store. Live rollback remains closed until a separate approval and rollback-witness table exists.
