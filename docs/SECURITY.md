...
- `relation-merge-review` is read-only. It audits applied merge witnesses for candidate remnants, remaining candidate edge references, self-loop residue, duplicate coalescing, and evidence-count preservation before later automation is allowed to trust relation-node merges.
