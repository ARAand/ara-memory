...
def verify_backup(path: Path) -> dict:
    with zipfile.ZipFile(path, "r") as zf:
        ...
        return {
            "path": str(path),
            "passed": not missing and integrity == "ok",
            "missing": missing,
...
def restore_backup(path: Path, target_root: Path, *, force: bool = False) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        return {
            ...
            "passed": post["passed"] and restored.schema_version() == verification["manifest"].get("schema_version"),
...
def _clear_memory_root(root: Path) -> None:
    allowed = {"memory.db", "ledger", "archive", "hot", "backups", "manifest.json"}
